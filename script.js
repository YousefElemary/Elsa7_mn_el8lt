...
const volunteerMembers = [
    ...
    { id: 14, name: 'مريم نصر الدين', role: 'أمين عام النادي' }
];
...
function renderVolunteerMembers() {
    ...
    const secretary = volunteerMembers.filter(m => m.role === 'أمين عام النادي');
        ...
        managerContainer.innerHTML = `
            ...
            ${secretary.map(m => `
                <div class="manager-card president-card">
                    ...
                    <div>
                        <div class="player-name">${m.name}</div>
                        <div class="player-role">أمين عام النادي</div>
                    </div>
                </div>
            `).join('')}
            ${manager.map(m => `
...
const CHATBOT_KNOWLEDGE = `
    ...
    - مدير التقنية (مطور الويب والمسؤول عن المنصة التقنية): يوسف محمد العمرى.
    - من صنع/برمج هذا الشات بوت والموقع؟ فريق البرمجة بقيادة يوسف العمري.
    - من هو فريق المبادرة؟ (سؤال شائع - جاوب بالفريق كامل دايماً، ولا تقتصر على اسم واحد): فريق متكامل يضم مؤسس المبادرة (عبدالرحمن أشرف الكومى)، منسقة المحتوى (منى نصر الدين سلامة)، مسئول التواصل والتسويق (مروان سمير جابر)، مسؤولة الإعلام (أسماء أشرف عبدالفتاح خليل)، مدير التقنية (يوسف محمد العمرى)، وفريق البرمجة بقيادته. بالإضافة إلى فريق المتطوعين.
    ...
    - فريق التطوع الأساسي (الظاهر في تشكيلة الموقع) يضم 13 شخصاً موزعين على أقسام: البرمجة، الميديا، التصوير الفوتوغرافي، مونتاج الفيديو، والتنظيم. بالإضافة إلى ذلك يوجد فريق احتياطي أوسع (حوالي 15 متطوعاً إضافياً) موزع على نفس اللجان.
    - رئيسة النادي (مسؤولة المتطوعين): مروة عبد الناصر.
    - أمين عام النادي: مريم نصر الدين.
    - المدير الفني (مسؤول التنظيم): محمد الكومي.

    🏢 **الشراكات والتعاون**
    - الشراكات مع مراكز الشباب: مركز شباب مدينة نصر، مركز التنمية الشبابية بمايو، مركز شباب الوايلي، مركز شباب الأميرية، مركز شباب التجمع الأول، مركز شباب الحي العاشر، مركز شباب زهراء المعادي، مركز شباب المنيل، مركز شباب نور المقطم، مركز شباب السواح، مركز التنمية الشبابية بمدينة بدر، مركز شباب كفر قنديل (الجيزة)، مركز شباب نزلة ترجم (الجيزة)، مركز شباب صول (الجيزة)، مركز تنمية شبابية أطفيح (الجيزة)، مركز شباب منشأة سليمان (الجيزة)، مركز شباب الكداية (الجيزة)، مركز تنمية شبابية مسجد موسى (الجيزة)، ومركز شباب قورص (المنوفية).
    - النوادي: نادي الترسانة.
    - الشراكات الخارجية: أكاديمية سياج، مؤسسة sys، مكتبة مصر العامة، جمعية أبناء دنفيق، حزب الجبهة الوطنية، إذاعة القاهرة الكبرى، مكتبة عالم سمسم، وإدارة المطرية.
    - ساهمت هذه الجهات في توفير القاعات واستضافة الفعاليات ودعم الوصول للفئات المستهدفة.
    ...
    - التحقق من المعلومات: لا تصدق أي إجابة من الذكاء الاصطناعي دون التأكد منها من مصدر موثوق، لأن هذه الأدوات قد تخطئ أو "تختلق" معلومات (ظاهرة تُعرف بـ"هلوسة الذكاء الاصطناعي" - AI Hallucination)، وتحدث عندما لا تملك الأداة بيانات كافية فتحاول سد الفجوة بمعلومات تبدو منطقية لكنها مختلقة.
    ...
    - هندسة الأوامر (Prompt Engineering): أفضل طريقة لكتابة أمر (برومبت) فعّال هي اتباع "المربع الذهبي": (1) تحديد دور الذكاء الاصطناعي (مثلاً: كاتب، معلم، مبرمج)، (2) توضيح المهمة والنتيجة المطلوبة بدقة، (3) توضيح السياق والجمهور المستهدف والهدف، (4) تحديد الأسلوب والتنسيق المطلوب.

    🎭 **التزييف العميق (Deepfake)**
    - هو محتوى رقمي (صوت أو فيديو) تم توليده أو تعديله بالكامل باستخدام الذكاء الاصطناعي لمحاكاة الواقع بدقة عالية، وقد يُستخدم لصناعة محتوى مضلل أو انتهاك الخصوصية.
    - علامات كشفه: رمش العين غير الطبيعي (بطيء أو غير متزامن)، تشوهات أو ألوان غير منطقية حول حواف الوجه والرقبة، وعدم تطابق حركة الفم مع الكلمات المنطوقة.

    🎣 **التصيد الإلكتروني (Phishing)**
    - من أخطر أساليب الاحتيال الرقمي، ولا يعتمد على اختراق الأجهزة بل على اختراق الثقة البشرية، وله عدة أشكال:
    - التصيد عبر البريد الإلكتروني: رسائل تبدو رسمية تطلب تحديث بيانات لوجود "مشكلة أمنية طارئة" تستدعي الضغط على رابط مرفق.
    - التصيد عبر SMS: رسائل للهاتف مثل "تم إيقاف بطاقتك البنكية، يرجى الاتصال بالرقم الآتي لتنشيطها".
    - التصيد الصوتي: مكالمات هاتفية ينتحل فيها المتصل صفة موظف خدمة عملاء أو دعم فني ويحاول الوصول لرموز التأكيد أو أرقام الحسابات.

    🔒 **البصمة الرقمية والخصوصية**
    - البصمة الرقمية هي السجل الكامل للأنشطة والبيانات التي يتركها الشخص أثناء استخدامه الإنترنت، وتنقسم لنشطة (بيانات يشاركها المستخدم بمحض إرادته كالمنشورات والتعليقات) وغير نشطة (تتجمع دون إدراك مباشر منه، وهي الأخطر).
    - خمس طرق أساسية لحماية الهوية الرقمية: كلمة مرور قوية جداً، تفعيل المصادقة الثنائية (2FA)، التحديث الدوري للأجهزة والتطبيقات، الحذر من الروابط المشبوهة، وحماية خصوصية البيانات الحساسة.

    😵 **التعفن الدماغي (Brain Rot)**
    - هو تراجع حاد في القدرات الذهنية والتركيز نتيجة الاستهلاك المفرط والعشوائي للمحتوى القصير والسطحي (فيديوهات السوشيال ميديا القصيرة جداً).
    - من علاماته: الملل السريع جداً، تشتت الانتباه، ضعف الإنتاجية، الضبابية الذهنية، اضطراب النوم، وإدمان التحقق من الهاتف باستمرار.
    - خطوات المواجهة: تحديد ساعات استخدام يومية للتطبيقات، القراءة الورقية العميقة، الرياضة والحركة، تحسين جودة النوم، وتعلم مهارة جديدة.

    🚨 **في حالة الوقوع في فخ رقمي (نصب، ابتزاز، انتحال شخصية)**
    - لا تحذف أي محتوى أو رسائل - الحفاظ على الأدلة دون تعديل أمر ملزم.
    - وثّق كل شيء بالتصوير الشامل (سكرين شوت) لكل الرسائل والروابط المباشرة.
    - اقطع الاتصال فوراً بالحظر (Block) للمرسل أو المحتال.
    - أبلغ الجهات المختصة فوراً.
    - أرقام الطوارئ الرسمية في مصر: 108 (مباحث الإنترنت)، 15115 (المجلس القومي للمرأة)، 16000 (خط نجدة الطفل).
`;
